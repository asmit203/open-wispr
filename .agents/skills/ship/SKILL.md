...
The deploy script calls `Codex -p` for release notes, which fails inside a Codex session due to nesting detection. Unset `Codex` to fix this:

```bash
env -u Codex bash scripts/deploy.sh <NEW_VERSION>
```
...
- Always use `env -u Codex` when running the deploy script
